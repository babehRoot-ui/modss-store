...
export default function HomePage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const { data: bans } = await supabase.from('banners').select('*').eq('is_active', true).order('order_position', { ascending: true });
    if (prods) {
      setProducts({
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => setCurrentBanner(i => (i + 1) % banners.length), 4000);
  ...
  const handlePay = async (e) => {
    e.preventDefault();
    if (!formData.phone || formData.phone.length < 10) { setError('Nomor WhatsApp harus valid'); return; }
  ...
  const closeModal = () => {
    setShowCheckout(false); setShowPayment(false); setCheckoutProduct(null);
    setPaymentData(null); setOrderData(null); setError('');
  ...
  return (
    <div className="min-h-screen">
      {/* Running Text */}
      <div className="bg-gradient-to-r from-accent/20 via-cyan-500/20 to-accent/20 border-b border-white/5 py-2 overflow-hidden">
        <div className="marquee-track animate-marquee">
          {[...Array(4)].map((_, i) => (
            <span key={i} className="inline-flex items-center gap-8 px-4 text-sm text-accent font-medium whitespace-nowrap">
              <span>BABEH DIGITAL STORE</span><span className="text-white/20">✦</span>
              <span>Panel Murah & Berkualitas</span><span className="text-white/20">✦</span>
              <span>Script Terupdate</span><span className="text-white/20">✦</span>
              <span>VPS High Performance</span><span className="text-white/20">✦</span>
              <span>Pengiriman Otomatis 24/7</span><span className="text-white/20">✦</span>
              <span>Payment QRIS Mudah</span><span className="text-white/20">✦</span>
            </span>
          ))}
        </div>
      </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        {banners.length > 0 && (
          <div className="relative rounded-2xl overflow-hidden mb-12 h-48 sm:h-64 md:h-80">
            {banners.map((b, i) => (
              <div key={b.id} className={`banner-slide absolute inset-0 ${i === currentBanner ? 'opacity-100' : 'opacity-0'}`}>
                <img src={b.image_url} alt={b.title || ''} className="w-full h-full object-cover" />
                <div className="absolute inset-0 bg-gradient-to-t from-dark-950/90 via-dark-950/30 to-transparent" />
                {b.title && <div className="absolute bottom-6 left-6 text-2xl sm:text-3xl font-bold text-white">{b.title}</div>}
              </div>
            ))}
            <div className="absolute bottom-4 right-4 flex gap-2">
              {banners.map((_, i) => (
                <button key={i} onClick={() => setCurrentBanner(i)} className={`w-2.5 h-2.5 rounded-full transition-all ${i === currentBanner ? 'bg-accent w-8' : 'bg-white/30 hover:bg-white/50'}`} />
              ))}
            </div>
          </div>
        )}

        {/* Hero Section (kalau tidak ada banner) */}
        {banners.length === 0 && (
          <div className="relative rounded-2xl overflow-hidden mb-12 p-8 sm:p-12 bg-gradient-to-br from-dark-800 to-dark-900 border border-white/5">
            ...
            <div className="relative">
              ...
              <h1 className="text-3xl sm:text-5xl font-black text-white mb-3 leading-tight">Toko Digital<br/><span className="text-transparent bg-clip-text bg-gradient-to-r from-accent to-cyan-400">Terpercaya</span></h1>
              <p className="text-gray-400 max-w-md text-sm sm:text-base">Jual panel, script, dan VPS dengan pengiriman otomatis. Bayar pakai QRIS, produk langsung terkirim!</p>
            </div>
          </div>
        )}
        ...
        {categories.map(cat => (
          <section key={cat.key} className="mb-12 animate-fade-in">
            ...
            {products[cat.key].length === 0 ? (
              <div className="glass-card p-8 text-center text-gray-500 text-sm">Belum ada produk {cat.label.toLowerCase()}, cek lagi nanti.</div>
            ) : (
      ...
      {/* Footer */}
      <footer className="border-t border-white/5 mt-16 py-8 text-center text-gray-500 text-xs">
        <p>&copy; {new Date().getFullYear()} BABEH DIGITAL STORE. All rights reserved.</p>
      </footer>

      {/* Modal Checkout */}
      {showCheckout && checkoutProduct && (
        <div className="fixed inset-0 z-[100] modal-overlay flex items-center justify-center p-4 animate-fade-in" onClick={closeModal}>
            ...
            {!showPayment ? (
              <>
                ...
                <div className="flex gap-4 mb-5 p-3 rounded-xl bg-dark-900/60 border border-white/5">
                  <img src={checkoutProduct.image_url || '/images/default-product.jpg'} alt={checkoutProduct.name} className="w-20 h-20 rounded-lg object-cover bg-dark-700" />
                  <div>
                    <p className="font-semibold text-white text-sm">{checkoutProduct.name}</p>
                    <p className="text-accent font-bold text-lg mt-1">{formatRupiah(checkoutProduct.price)}</p>
                    {checkoutProduct.badge && <span className={`badge mt-1 inline-block badge-${checkoutProduct.badge.toLowerCase()}`}>{checkoutProduct.badge}</span>}
                  </div>
                </div>
                <form onSubmit={handlePay} className="space-y-4">
                  <div>
                    <label className="block text-xs text-gray-400 mb-1.5">Nama (opsional)</label>
                    <input type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="input-field" placeholder="Nama kamu" />
                  </div>
                  <div>
                    <label className="block text-xs text-gray-400 mb-1.5">Nomor WhatsApp <span className="text-red-400">*</span></label>
                    <input type="tel" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="input-field" placeholder="08xxxxxxxxxx" required />
                  </div>
                ...
                <div className="text-center">
                  ...
                  {paymentData?.qr_string && (
                    <div className="inline-block p-3 bg-white rounded-2xl mb-4">
                      <img src={`https://api.qrserver.com/v1/create-qr-code/?data=${encodeURIComponent(paymentData.qr_string)}&size=220x220&color=000000`} alt="QRIS" className="w-52 h-52" />
                    </div>
                  )}
                  <p className="font-bold text-white text-xl mb-1">{formatRupiah(checkoutProduct.price)}</p>
                  <p className="text-xs text-gray-400 mb-4">Scan QR di atas pakai aplikasi e-wallet</p>
  ...
  return (
    <div className="product-card glass-card overflow-hidden cursor-pointer group" onClick={() => onBuy(product)}>
      <div className="relative h-44 bg-dark-700 overflow-hidden">
        <img src={product.image_url || '/images/default-product.jpg'} alt={product.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
        <div className="absolute inset-0 bg-gradient-to-t from-dark-900/80 to-transparent" />
        {product.badge && (
          <span className={`badge badge-${product.badge.toLowerCase()} absolute top-3 left-3`}>{product.badge}</span>
        )}
        <span className="absolute top-3 right-3 text-xs bg-dark-900/70 backdrop-blur text-gray-300 px-2 py-1 rounded-lg border border-white/10">{catInfo.icon} {catInfo.label}</span>
      </div>
      <div className="p-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-gray-500 bg-dark-900/50 px-2 py-1 rounded">Stok: {product.stock}</span>
        </div>
